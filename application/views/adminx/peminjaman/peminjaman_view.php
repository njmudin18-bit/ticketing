...
<body data-theme="default" data-layout="fluid" data-sidebar-position="left" data-sidebar-layout="default">
                    ...
                    <table id="datatables-reponsive" class="table table-striped" width="150%">
                      <thead>
                        <tr class="bg-primary">
                          ...
                          <th class="text-center text-white">No Peminjaman</th>
                          ...
                          <th class="text-center text-white">Tanggal Peminjaman</th>
                          <th class="text-center text-white">Tanggal Dikembalikan</th>
        ...
        <div class="modal-body">
          <form action="" method="post" id="registerForm">
            ...
            <div class="form-group row mb-3">
              ...
              <div class="col-sm-8">
                <input type="date" class="input-sm form-control" id="tglAwal" name="tglAwal" />
                <span class="help-block"></span>
              </div>
            </div>
            ...
            <div class="form-group row mb-3">
              ...
              <div class="col-sm-8">
                <input type="date" class="input-sm form-control" id="tglAkhir" name="tglAkhir" />
                <span class="help-block"></span>
              </div>
            </div>
  ...
  function getNoRequest(callback) {
    $.ajax({
      url: '<?php echo base_url(); ?>peminjaman/getNoRequest',
      type: 'POST',
  ...
  function openModalDelete(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: '<?php echo base_url(); ?>peminjaman/delete_data/' + id,
          type: 'DELETE',
          error: function() {
  ...
  function gantiStatus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: '<?php echo base_url(); ?>peminjaman/update_data_status',
          type: 'POST',
          data: {
            status: pilihStatus,
            kode: id
  ...
  function save() {
    ...
    if (save_method == 'add') {
      $("#pass_div").show();
      url = "<?php echo base_url(); ?>peminjaman/add_data";
      status = "tambah data";
    } else {
      $("#pass_div").hide();
      url = "<?php echo base_url(); ?>peminjaman/update_data";
      status = "update data"
    }
  ...
  function edit(id) {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>peminjaman/edit_data/" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data) {
        if (data.status == 'forbidden') {
          ...
          $('[name="keterangan"]').val(data.keterangan_pinjaman);
          $('[name="tglAwal"]').val(data.tgl_pinjam);
          $('[name="tglAkhir"]').val(data.tgl_kembali);
          $('#modalForm').modal('show');
          $('#no_request').prop('disabled', true);
  ...
  function approve_spv_it(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        console.log(pilihStatus + ' ' + id);
        $.ajax({
          url: '<?php echo base_url(); ?>peminjaman/update_approveit/' + id,
          type: 'POST',
          data: {
            approve_it: pilihStatus,
            kode: id
          },
          error: function() {
            alert('Something is wrong');
  ...
  document.addEventListener("DOMContentLoaded", function(event) {
    ...
    table = $('#datatables-reponsive').DataTable({
      ...
      "order": [],
      "ajax": {
        "url": "<?php echo base_url(); ?>peminjaman/show",
        "type": "POST",
        "data": function(data) {
      ...
      "aoColumns": [{
        ...
        {
          "Tanggal Dikembalikan": "Tanggal Dikembalikan",
          "sClass": "text-center"
        },
        {
          "Created": "Created",
