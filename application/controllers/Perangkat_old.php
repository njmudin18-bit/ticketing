...
class Perangkat extends CI_Controller {
	...
	public function perangkat_add()
  {
      ...
      //$dept       = $this->departments->get_by_id($dept_id);
      //$dept_name  = str_replace(" ", "", $dept->nama_dept);
      $kode       = $this->perangkat->auto_generate($company_id, $dept_id);
      //$kode       = $this->perangkat->GenerateID($company_id, $dept_id);
      //echo $kode; exit;
  ...
  public function perangkat_list()
  {
  	$list = $this->perangkat->get_datatables();
		...
		foreach ($list as $perangkat) {
			...
			$row[] = $no;
			//add html for action
			$row[] = '<a href="javascript:void(0)" onclick="edit('."'".$perangkat->id."'".')"
      ...
      $row[] = $perangkat->status;
			$row[] = $perangkat->nama;
			$row[] = $perangkat->nama_dept; //$perangkat->DivisionID; //$perangkat->nama_dept;
			$row[] = $perangkat->jenis;
			$row[] = $perangkat->user_id;
  ...
  public function perangkat_edit($id)
	...
	public function perangkat_update()
	{
      ...
      // $dept       = $this->departments->get_by_id($dept_id);
      // $dept_name  = str_replace(" ", "", $dept->nama_dept);
      // $kode       = $this->perangkat->auto_generate($company_id, $dept_name);
      // echo $kode;
      // exit;
